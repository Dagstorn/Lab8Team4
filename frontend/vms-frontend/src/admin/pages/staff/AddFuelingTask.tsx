...
const AddFuelingTask = () => {
    ...
    const onSubmit = async (values: FieldValues) => {
        ...
        const response = await sendRequest('/api/staff/tasks/fueling/', 'post', {
            Authorization: `Bearer ${auth.tokens.access}`
        }, values)
        if (response) {
            reset();
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-4">Add fueling task </h1>
            <Separator />
