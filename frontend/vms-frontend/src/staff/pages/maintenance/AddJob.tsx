...
import { RepairPart, Vehicle } from "@/shared/types/types";
...
const AddJob = () => {
    ...
    const getVehicle = async () => {
        if (vehicleId) {
            ...
            if (responseData) {
                ...
                setValue('vehicle', responseData.id);
    ...
    const onSubmit = async (values: FieldValues) => {
        clearError();
        // add reapairing parts to form data
        values.repair_parts = repairParts;
        // send task data to backend
        const response = await sendRequest('/api/maintenance/jobs/', 'post', {
            Authorization: `Bearer ${auth.tokens.access}`
        }, values)
        if (response) {
            reset();
    ...
    return (
        <div>
            ...
            {vehicle && <div className="mt-4 flex flex-col text-base">
                <div className="flex">
                    <span className="w-2/6 font-bold mr-2">Make & model:</span>
                ...
                <form action="" onSubmit={handleSubmit(onSubmit)}>
                    <div className="mt-4 flex items-center">
                        <label className="font-bold mr-4">Maintenance Job type</label>
                        <Select
                            {...register("type", {
